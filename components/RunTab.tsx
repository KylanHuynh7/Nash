...
import { balanceTeams, type BalanceResult, type Constraint } from "@/lib/balance";
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="mb-2.5 flex items-baseline justify-between">
          ...
          <div className="flex items-center gap-3 text-sm">
            ...
            <button
              onClick={() => {
                setPresent(
                  here.length === roster.length ? new Set() : new Set(roster.map((p) => p.id)),
                );
                setResult(null);
              }}
      ...
      <section>
        ...
        >
          <span>
            Pairing rules{rules.length > 0 && ` · ${rules.length}`}
          </span>
          ...
          <div className="mt-2 space-y-2 rounded-2xl border border-line bg-surface p-3">
            {rules.length === 0 && (
              <p className="px-1 py-1 text-xs leading-relaxed text-muted">
                Force two people onto the same team, or keep them apart. Rules only
                apply when both players are marked here.
              </p>
            )}
            {rules.map((rule, i) => {
              ...
              return (
                ...
                >
                  <span className="flex-1">
                    {a?.name} <span className="text-muted">
              ...
              <div key={i} className="rounded-2xl border border-line bg-surface p-4">
                <div className="mb-3 flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2 text-xs text-muted">
                    <span className="font-mono tabular-nums">avg {team.average}</span>
...
function RuleBuilder({
  ...
  return (
    <div className="flex flex-wrap items-center gap-2 pt-1">
      ...
      <button
        type="button"
        onClick={() => setKind((k) => (k === "together" ? "apart" : "together"))}

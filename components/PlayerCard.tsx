...
import { getNbaComp, type RosterEntry } from "@/app/actions";
...
export default function PlayerCard({
  ...
  config: SportConfig;
  player: RosterEntry;
  /** The whole roster, which the signature badges measure him against. */
  roster: RosterEntry[];
  ...
  useEffect(() => {
    ...
    if (config.comps) {
      getNbaComp(config.id, player.id)
        ...
        // A missing line beats a card that will not open, so this degrades to
